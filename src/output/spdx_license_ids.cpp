...
/// Vendored SPDX License List short identifiers (lower-cased key -> the
/// list's own canonical spelling), pinned to license-list-data tag v3.28.0.
/// Includes deprecated ids (still schema-valid, just discouraged) so a
/// component carrying one is still recognized rather than needlessly
/// wrapped as a LicenseRef-. Re-fetch to refresh (review the diff before
/// committing it, per rule 3 -- a refresh changes writer output):
///   curl -fsSL -o /tmp/spdx_licenses.json
///     https://raw.githubusercontent.com/spdx/license-list-data/vNEW/json/licenses.json
const std::map<std::string_view, std::string_view>& spdx_license_ids_by_lowercase_id()
{
  static const std::map<std::string_view, std::string_view> kIds{
      {"0bsd", "0BSD"},
...
std::optional<std::string_view> canonical_spdx_license_id(std::string_view license_text)
{
  const std::string lowered = core::to_lower_ascii(core::trimmed_view(license_text));
  const auto& ids = spdx_license_ids_by_lowercase_id();
  const auto found = ids.find(std::string_view(lowered));
  if (found == ids.end())
  {
    return std::nullopt;
  }
